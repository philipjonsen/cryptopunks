...
contract('CryptoPunksMarket-buySellRemoveFromSale', function (accounts) {
  ...
  it('can not buy a punk for too little money', async function () {
    ...
    const ethBalance = await web3.eth.getBalance(accounts[1])
  ...
  it('can buy a punk that is for sale', async function () {
    ...
    const balanceToWidthdraw = await contract.pendingWithdrawals.call(
      accounts[0]
    )
    assert.equal(
  ...
  it('can withdraw money from sale', async function () {
    const contract = await CryptoPunksMarket.deployed()
    const accountBalancePrev = await web3.eth.getBalance(accounts[0])
  ...
  it('can offer for sale then withdraw', async function () {
    ...
    const offer = await contract.punksOfferedForSale.call(1)
    console.log('Offer: ' + offer)
    ...
    const offerPost = await contract.punksOfferedForSale.call(1)
    console.log('Offer: ' + offer)
  ...
  it('can withdraw money from sale to specific account', async function () {
    const contract = await CryptoPunksMarket.deployed()
    const accountBalancePrev = await web3.eth.getBalance(accounts[1])
